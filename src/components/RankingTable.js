...
export default class RankingTable extends React.Component {
    ...
    const { order, orderBy, conferenceFilter, selectedTeamMap, doOrderBy, showModal } = this.props;  
    ...
    const players = playerData.map((item, index)=>{
      return <Player data={ item } key={ item._id } rank={ index } orderBy={ orderBy } showModal={ showModal } />
    }); 
